...
int process_rdma_cm_event(struct rdma_event_channel *echannel, 
    ...
    int ret = 1;
	ret = rdma_get_cm_event(echannel, cm_event); //block until event coming
    if(ret){
...
int process_work_completion_events (struct ibv_comp_channel *comp_channel, 
   ...
   ret = ibv_req_notify_cq(cq_ptr,0);
    ...
    complete_wc = 0;
    do{
        ret = ibv_poll_cq(cq_ptr,max_wc-complete_wc,wc+complete_wc);
        if(ret < 0){
        ...
        complete_wc += ret;
    }while(complete_wc < max_wc);

    /* Now we check validity and status of I/O work completions */
    for(i = 0 ;i < complete_wc;i++){
