...
static int client_pre_post_recv_buffer()
{
	...
	server_recv_wr.wr_id = RECV_WRID;
...
static int client_xchange_metadata_with_server()
{
	...
	client_send_wr.wr_id = SEND_WRID;
 ...
 * 3) RDMA read from remote bufer -> dst
 */ 
static int client_remote_memory_write_read_ops() 
{
	struct ibv_wc wc;
	int ret = -1;
...
/* This function does :
 * 1) Prepare memory buffers for RDMA operations 
 * 2) Do RDMA ops according to arguments
 * arguments:
 * ops_type:RDMA operation type:
 * WRITE:write data from cli to ser
 * WRITE_IMMDT:write data from cli to ser with immdt
 * READ:read data from  ser to cli
 * ATOMIC:
 * 	ATOMIC_COMSWAP:compare and swap
 *  ATOMIC_FETCHADD:fetch and add
 */ 

 static int client_remote_memory_ops(int ops_type,...) {
	struct ibv_wc wc;
	int ret = -1;
	client_dst_mr = rdma_buffer_register(pd,
			dst,
	...
	va_list arg;
	va_start(arg,ops_type);
	//判断操作类型并下发相应请求
	bzero(&client_send_wr, sizeof(client_send_wr));
	switch(ops_type){
	case WRITE_IMMDT:
		//获取额外参数
		uint32_t immdt_data = va_arg(arg, uint32_t);
		client_send_wr.imm_data = htonl(immdt_data);
		va_end(arg);
	case WRITE:
		client_send_sge.addr = (uint64_t) client_src_mr->addr;
		client_send_sge.length = (uint32_t) client_src_mr->length;
		...
		client_send_wr.wr_id = WRITE_WRID;
		client_send_wr.sg_list = &client_send_sge;
		client_send_wr.num_sge = 1;
		client_send_wr.opcode = (ops_type==WRITE ? IBV_WR_RDMA_WRITE:IBV_WR_RDMA_WRITE_WITH_IMM);
		client_send_wr.send_flags = IBV_SEND_SIGNALED;
		client_send_wr.wr.rdma.remote_addr = server_metadata_attr.address;
		client_send_wr.wr.rdma.rkey = server_metadata_attr.stag.remote_stag;
		break;
	case READ:
		...
		client_send_wr.wr_id = READ_WRID;
		client_send_wr.sg_list = &client_send_sge;
		client_send_wr.num_sge = 1;
		client_send_wr.opcode = IBV_WR_RDMA_READ;
		client_send_wr.send_flags = IBV_SEND_SIGNALED;
	...
	case ATOMIC_COMSWAP:
		...
		client_send_wr.wr_id = (ops_type==ATOMIC_COMSWAP?ATOMIC_COMSWAP_WRID:ATOMIC_FETCHADD_WRID);
		client_send_wr.sg_list = &client_send_sge;
		client_send_wr.num_sge = 1;
		client_send_wr.opcode = (ops_type==ATOMIC_COMSWAP ? IBV_WR_ATOMIC_CMP_AND_SWP:IBV_WR_ATOMIC_FETCH_AND_ADD);
		client_send_wr.send_flags = IBV_SEND_SIGNALED;
		client_send_wr.wr.atomic.remote_addr = server_metadata_attr.address;
		client_send_wr.wr.atomic.rkey = server_metadata_attr.stag.remote_stag;

		//获取额外参数
		client_send_wr.wr.atomic.compare_add = va_arg(arg,uint64_t);
		if(ops_type == ATOMIC_COMSWAP){
			client_send_wr.wr.atomic.swap = va_arg(arg,uint64_t);
		}
		va_end(arg);
	    break;
		default:
		  rdma_error("Invalid ops_type\n");
		  return -1;
	};
	...
	if(ret != 1){
		rdma_error("We failed to get  work completions , ret = %d \n",
				ret);
		return ret;
	}
	  ...
	  //加入对事件的处理判断
        ...
        case SEND_WRID:
		  //客户端的元数据已发送完成
		  debug("Client sent server its buffer info\n");
          break;
        case WRITE_WRID:
			debug("Client finished write request\n");
          break;
		case WRITE_IMMDT_WRID:
			debug("Client finished write_immdt request\n");
		  break;
        case READ_WRID:
			debug("Client finished read request\n");
			//打印数据
			printf("Client read data: %s\n", dst);
         break;
        case ATOMIC_COMSWAP_WRID:
			debug("Client finished atomic_comswap request\n");
			printf("Server original data: %s\n", dst);
			//添加客户端的预设结果------------------//
         break;
		case ATOMIC_FETCHADD_WRID:
			debug("Client finished atomic_fetchadd request\n");
			printf("Server original data: %s\n", dst);
			//添加客户端的预设结果------------------//
		 break;
        default:
		  rdma_error("Unknown wr_id in work completion\n");
		  return -1;
        };
...
int main(int argc, char **argv) {
	...
	server_sockaddr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);  //默认回环地址

	src = calloc(size,1);
	dst = calloc(size,1);
	/* Parse Command Line Arguments */
	while ((option = getopt(argc, argv, "s:a:p:o:")) != -1) {
		switch (option) {
			case 's':  //指定本端内存的缓存内容
				printf("Passed string is : %s , with count %u \n", 
						optarg, 
						(unsigned int) strlen(optarg));
				//src = calloc(size, 1);  //本端内存
				if (!src) {
				...
				//dst = calloc(size, 1);  //对端传输内容在本地的缓存
				if (!dst) {
			...
			case 'a':
				...
				ret = get_addr(optarg, (struct sockaddr*) &server_sockaddr);//获取IP地址
				if (ret) {
	...
	if(ret){
		rdma_error("Failed to post client recv, ret = %d \n", ret);
		return ret;
	}
