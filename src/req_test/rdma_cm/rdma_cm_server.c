...
static int accept_client_connection()
{
	 ...
	 * yet because we have to do lots of resource pre-allocation */
       ...
       ret = rdma_accept(cm_client_id, &conn_param); //在连接套接字上接收客户端连接
       if (ret) {
...
static int send_server_metadata_to_client() 
{
	...
	*/
       ...
       server_send_wr.wr_id = SEND_WRID;
...
int main(int argc, char **argv){
	...
	while ((option = getopt(argc, argv, "a:p:o:")) != -1) {
		switch (option) {
			case 'a':
     ...
     //2.准备资源(RC服务类型下，对于新的连接需要创建新的QP等资源进行通信)
     ret = setup_resources();
     if(ret){
     ...
     if(ret){
        rdma_error("Failed to send server metadata cleanly, ret = %d \n", ret);
		return ret;
     }


    //poll cq循环,循环一段时间后退出
    float time =.0;int n = 0;
    ...
    do{
        ...
        if(wc.opcode == IBV_WC_RECV_RDMA_WITH_IMM){
            ...
            printf("Server recv immdt_data:%d\n",imm_data);  
        }
timeout:
        gettimeofday(&cur_time,NULL);
        time =((cur_time.tv_sec * 1000000 + cur_time.tv_usec) - (start_time.tv_sec * 1000000 + start_time.tv_usec)) / 1000000;
    }while(time < 3);

    debug("poll CQ loop exit\n");
    //打印服务端内存数据
    char *res_data=(char *)calloc(1,sizeof(server_buffer_mr->length));
    memcpy(res_data,server_buffer_mr->addr,server_buffer_mr->length);
    printf("Server get data:%s", res_data);
