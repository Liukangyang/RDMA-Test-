...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        c = getopt_long(argc, argv, "p:d:i:s:m:r:n:l:eg:otca:h",
				long_options, NULL);
    ...
    ctx = init_ctx(ib_dev, size,rx_depth, ib_port,use_event,access_flags,IBV_ACCESS_REMOTE_READ);
    ...
    //服务端存储数据
    if(!client){
        sprintf((char *)ctx->buf,"test data:rdma read test\n\0");
    }
    //3.socket建链
    if(client){  //客户端
    ...
    if(client){
        //read请求
		printMsg("post read req\n");
        if(post_read(ctx,rem_dest)){
            fprintf(stderr, "Couldn't post send\n");
			return 1;
        }
        ctx->pending |= READ_WRID; 
    ...
    do{
        ...
        for(i = 0;i < ne;i++){
            ...
            switch((int)wr_id){
                ...
                case RECV_WRID:
                rcnt++;
                printMsg("Recv req complete,recv data:%s\n",(char *)ctx->buf);
                break;
                ...
                if(opcode == IBV_WC_RDMA_WRITE){
                    scnt++;
				    double usec = (end.tv_sec*1000000+end.tv_usec)-(start.tv_sec*1000000+start.tv_usec);//时间单位us
                    printMsg("Write req complete for %.4f us\n",usec);

                }
                else if(opcode == IBV_WC_RECV_RDMA_WITH_IMM){//imm_data at reciver
                    rcnt++;
                    //获取imma_data
                ...
                if(opcode == IBV_WC_RDMA_READ){
                    rcnt++;
                    double usec = (end.tv_sec*1000000+end.tv_usec)-(start.tv_sec*1000000+start.tv_usec);//时间单位us
                    printMsg("Read req complete for %.4f us\n",usec);
                }
				default:
