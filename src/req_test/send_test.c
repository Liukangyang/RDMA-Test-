...
int main(int argc, char *argv[]){
    ...
    static struct option long_options[] = {
            ...
            { .name = "addr",     .has_arg = 1, .val = 'a'},   //server-ip-addr
    ...
    while(1){
        ...
        c = getopt_long(argc, argv, "p:d:i:s:m:r:n:l:eg:otca:I:h",
				long_options, NULL);
    ...
    //2.执行初始化操作
    ctx = init_ctx(ib_dev, size,rx_depth, ib_port,use_event,IBV_ACCESS_LOCAL_WRITE,0);
    if(!ctx){
    ...
    my_dest.qpn = ctx->qp->qp_num;      //psn
    my_dest.psn = lrand48() & 0xffffff; //随机起始PSN
	...
	printMsg(" remote address: LID 0x%04x, QPN 0x%06x, PSN 0x%06x, GID %s\n",
    ...
    if(client){
		...
		printMsg("post send req\n");
        if(post_send(ctx,immdt_flag,immdt_data)){
